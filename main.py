...
def process_and_show_semantic_analysis_results(audio_file, transcribed, transcribed_text, model):
    if not transcribed and audio_file:
        ...
        transcribed_text = vas.transcribe_audio_file(audio_file)
    ...
    if model == 'All':
        ...
        result = vas.vas.perform_sentiment_analysis(transcribed_text, return_all, model)
        ...
        for k in list(result.keys()):
            ...
            if k == 'flair':
                ...
                col1.subheader(f"{vas.get_sentiment_emoji(sentiment_label.lower())} {sentiment_label}")
            ...
            if k == 'vader':
                ...
                col2.subheader(f"{vas.get_sentiment_emoji(sentiment_label.lower())} {sentiment_label}")
            ...
            if k == 'textblob':
                ...
                col3.subheader(f"{vas.get_sentiment_emoji(sentiment_label.lower())} {sentiment_label}")
            ...
            if k == 'roberta':
                ...
                col4.subheader(f"{vas.get_sentiment_emoji(sentiment_label.lower())} {sentiment_label}")
            ...
            if k == 'distilbert':
                ...
                col5.subheader(f"{vas.get_sentiment_emoji(sentiment_label.lower())} {sentiment_label}")
        ...
        sentiment_label, sentiment_score = vas.perform_sentiment_analysis(transcribed_text, return_all, model)
        if(sentiment_label == 'error'):
            ...
            st.markdown("*" + vas.print_sentiments(sentiment_label, sentiment_score) + "*")
...
def process_and_show_text_classification_results(audio_file, transcribed, transcribed_text):
    ...
    return_all=False
    if return_all:
        resultDict = vas.perform_text_classification_using_bhadresh_savani(transcribed_text, return_all)
        rsultDictionary=vas.convertToNewDictionary(resultDict)
        sentimental_results = []
        ...
        sentiment_label, sentiment_score = vas.perform_text_classification_using_bhadresh_savani(transcribed_text, return_all)
        st.markdown("*"+vas.print_sentiments(sentiment_label, sentiment_score)+ "*")
...
def display_sentiment_results(sentiment_results, option):
    ...
    for sentiment, score in sentiment_results.items():
        emoji = vas.get_sentiment_emoji(sentiment)
...
def main():
    ...
    if action == 'Plain Text':
        if analyse and len(text)>10:
            ...
            #     for sentence, assessment in sentiment_results.items():
            #         st.write(sentence)
            #         st.write(assessment)
            #         st.write('--- - - - - - - - - - - - - - - - - - - - - - - --------------------------------')
