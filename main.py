...
def display_all_results_for_multiple_sentence(model, return_all, transcribed_text):
    ...
    for k in list(result.keys()):
        ...
        if k == 'flair':
            ...
            col1.subheader(f"{myu.get_sentiment_emoji(myu,sentiment_label.lower())} {sentiment_label}")
        ...
        if k == 'vader':
            ...
            col2.subheader(f"{myu.get_sentiment_emoji(myu, sentiment_label.lower())} {sentiment_label}")
        ...
        if k == 'textblob':
            ...
            col3.subheader(f"{myu.get_sentiment_emoji(myu, sentiment_label.lower())} {sentiment_label}")
...
def process_and_show_sentimental_analysis_results(audio_file, transcribed, transcribed_text, model):
    ...
    if model == 'All':
        if not isinstance(transcribed_text, str):
            display_all_results_for_multiple_sentence(model, return_all, transcribed_text)
        else:  # 'All-SS'
            display_all_results_for_one_sentence(model, return_all, transcribed_text)
    else:
        ...
        if (sentiment_label == 'error'):
            ...
            # print(sentiment_label)
            # print(sentiment_score)
            st.markdown('*'+f'{myu.get_sentiment_emoji(myu, sentiment_label)}{sentiment_label} (Score: {sentiment_score})'+'*')
...
def process_and_show_new_sentimental_analysis_results(audio_file, transcribed, transcribed_text, model):
    ...
    if 'All' in model:
        ...
        print(f'model is {model}')
        print(transcribed_text)
        sentiment_label, sentiment_score = voiceAnalysisServices.perform_sentiment_analysis(transcribed_text,
                                                                                            return_all, model)
        print(sentiment_label)
        print(sentiment_score)
        if (sentiment_label == 'error'):
            ...
            print(sentiment_score)
            st.markdown(
                '*' + f'{myu.get_sentiment_emoji(myu, sentiment_label)}{sentiment_label} (Score: {sentiment_score})' + '*')


def display_all_results_for_one_sentence(model, return_all, transcribed_text):
    ...
    for k in list(result.keys()):
        ...
        if k == 'distilbert':
            col4.markdown("<font size='5' align='center'> Model: {} </font>".format(k.upper()), unsafe_allow_html=True)
            col4.markdown("<font size='4' > Score: {} </font>".format(sentiment_score), unsafe_allow_html=True)
            col4.subheader(f"{myu.get_sentiment_emoji(myu, sentiment_label.lower())} {sentiment_label}")
...
def process_and_show_text_classification_results(audio_file, transcribed, transcribed_text):
    ...
    print(transcribed_text)
    t_text = ' '.join(transcribed_text)
    print(t_text)
    st.header("Text Classification Results Analysis(Bert-base-uncased-emotion)")
    result = voiceAnalysisServices.perform_text_classification_using_bhadresh_savani(
                                                                pd.DataFrame({"text": [transcribed_text]}), return_all=False)
    # print('result')
    # print(result)
    if len(result) == 1:
        sentiment = result.loc[0]['sentiment']
        score = result.loc[0]['polarity']
        st.markdown("*" + f'{myu.get_sentiment_emoji(myu, sentiment)}{sentiment} (Score: {score})' + "*")
...
def process_and_plot_text_classification_results(audio_file, transcribed, transcribed_text, return_all):
    ...
    if return_all:
        ...
        result = voiceAnalysisServices.perform_text_classification_using_bhadresh_savani(
            transcribed_text, return_all)
        if len(result) == 1:
            sentiment_label = result.loc[0]['sentiment']
            sentiment_score = result.loc[0]['polarity']
...
def main():
    ...
    if action in 'Sample Audio':
        ...
        with st.sidebar:
            ...
            recorded_audio_in_bytes = ars.audio_recorder(text="Click to Record", pause_threshold=3.0)
        try:
        ...
        if model_predict != 'All':
            ...
            if text_csv_file:
                print("2 text_csv_file")
                print(text_csv_file.type)
            if upload_button_csv_file:
                print("3 upload_button_csv_file")
            if text_csv_file and (text_csv_file.type == 'application/vnd.ms-excel' or text_csv_file.type == 'text/csv' or text_csv_file.type == 'text/plain') and upload_button_csv_file:
                try:
                    with st.spinner('Processing...'):
                        ...
                        if model_predict != 'savani' and model_predict != 'All':  # dilbert
                            for text in df1.iloc[:, 0]:
                                rsult = voiceAnalysisServices.perform_sentiment_analysis(text, return_all=False,
                                                                                         model=model_predict)
                            ...
                            df = voiceAnalysisServices.perform_sentiment_analysis(df1, return_all=True,
                                                                                  model=model_predict,
                                                                                  isFileUpload=True)
                            progress_bar.progress(80, 'Plotting...!!')
                            ...
                            print(model_predict)
                            df = voiceAnalysisServices.perform_sentiment_analysis(df1, return_all=True, model='savani',
                                                                                  isFileUpload=True)
                            progress_bar.progress(50, 'Plotting...!!')
                            plot_to_charts_savani(df)
                            df = voiceAnalysisServices.perform_sentiment_analysis(df1, return_all=True, model='samLowe',
                                                                                  isFileUpload=True)
                            progress_bar.progress(50, 'Plotting...!!')
                            plot_to_charts_savani(df)
                            progress_bar.progress(100, 'Done, Finished Processing!!')
